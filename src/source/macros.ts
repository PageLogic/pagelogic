...
export const MACRO_TAG = DIRECTIVE_TAG_PREFIX + 'MACRO';
export const MACRO_TAG_ATTR = 'tag';
export const SLOT_TAG = DIRECTIVE_TAG_PREFIX + 'SLOT';
...
export class Macros {
  ...
  protected collectMacros(
    source: Source, p: html.Element, nesting: number, ret: Definitions
  ): Definitions {
  ...
  protected collectMacro(
    source: Source, p: html.Element, e: html.Element, nesting: number,
    macros: Definitions
  ): Definition | null {
    const tagAttr = e.getAttributeNode(MACRO_TAG_ATTR);
    if (!tagAttr || typeof tagAttr.value !== 'string') {
      source.addError('warning', `bad or missing "${MACRO_TAG_ATTR}" attribute`, e.loc);
      return null;
    }
  ...
  protected removeMacros(source: Source, macros: Definitions) {
  ...
  protected collectSlots(
    node: html.Element, source: Source
  ...
  protected expandMacros(
    source: Source, p: html.Element, nesting: number, macros: Definitions
  ) {
  ...
  protected expandMacro(
    source: Source, p: html.Element, e: html.Element, def: Definition,
