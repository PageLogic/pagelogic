...
function parseAtomicText(p: dom.Element, src: Source, i1: number, i2: number, errors: PageError[]) {
  ...
  if (k < 0 || k >= i2) {
    // static text
    new dom.Text(p.doc, p, s.substring(i1, i2), src.loc(i1, i2));
    return;
  }
  ...
  if (exps.length === 1) {
    new dom.Text(p.doc, p, exps[0], src.loc(i1, i2));
    return;
  }
  ...
  new dom.Text(p.doc, p, exp, src.loc(i1, i2));
}
...
function parseSplittableText(p: dom.Element, src: Source, i1: number, i2: number, errors: PageError[]) {
  ...
  for (let j1 = i1; j1 < i2;) {
    let j2 = s.indexOf(LEXP, j1);
    if (j2 < 0 || j2 >= i2) {
      new dom.Text(p.doc, p, s.substring(j1, i2), src.loc(j1, i2));
      break;
    }
    if (j2 > j1) {
      new dom.Text(p.doc, p, s.substring(j1, j2), src.loc(j1, j2));
      j1 = j2;
    }
    const j0 = j2;
    j2 += LEXP.length;
    ...
    new dom.Text(p.doc, p, exp, src.loc(j0, j1));
  }
}
