...
export class Preprocessor {
  ...
  constructor(docroot: string) {
    ...
    this.macroProcessor = new Macros();
  }
  ...
  async load(fname: string): Promise<Source> {
    ...
    if (source && !source?.errors.length) {
      this.macroProcessor.processMacros(source);
    }
    return source ?? dummy;
  }
  ...
  protected async processLiteralInclude(
    ...
    const e = new dom.Element(d.node.doc, null, as, d.node.loc);
    new dom.Text(e.doc, e, loaded.text, d.node.loc, false);
